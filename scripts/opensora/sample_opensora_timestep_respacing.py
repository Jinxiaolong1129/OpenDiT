            ...
            if not use_dist or coordinator.is_master():
                for idx, sample in enumerate(samples):
                    ...
                    if hasattr(args, "timestep_respacing"):
                        print("timestep_respacing 是 args 的属性")
                    else:
                        print("timestep_respacing 不是 args 的属性")
                    sample_name_suffix = f"_{sample_idx}_{batch_prompts_raw[idx][:30]}_{args.timestep_respacing}"
